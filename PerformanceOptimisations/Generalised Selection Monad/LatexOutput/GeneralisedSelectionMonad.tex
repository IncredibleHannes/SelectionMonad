...
\setlength\parindent{0pt} % commented out - JG
...
\lstdefinelanguage{Haskell}{
  ...
  emph={[2]Ord, J, K, Decision, G, Bool, Int, Char, Show,String,ghci}, emphstyle={[2]\color{Mahogany}},
...
intermediate result. Note that \(\mathbin{{*}{*}{*}}\) is the operator
that given two functions \(f: A \rightarrow A'\) and
\(g: B \rightarrow B'\) it returns a function of type
\((A,B) \rightarrow (A',B')\), where \(f\) is applied to the first
element of the tuple and \(g\) is applied to the second element of the
tuple.

\qquad With the free theorem for \(K\), the remaining portion of the
...
\qquad By using the free theorem for \(G\), it becomes clear that
selection functions designed for the \(K\) type can be directly embedded
into the \(G\) structure:
...
Further, to establish that selection functions of type \(G\) can be
...
\section{G forms a monad}\label{g-forms-a-monad}

The formation of the monad for \(G\) follows a straightforward
definition:
...
In this context, given a selection function \(e\) of type \(G_{R,A}\), a
function \(f\) of type \(A \rightarrow G_{R,A}\), and a property
...
straightforward return suffices. The return for the \(G\) type is
defined as follows:
...
Now it is possible calculate the \(bindG\) implementation for \(G\) with
the \(j2g\) and \(g2j\) operators and the previously introduced
theorems:
